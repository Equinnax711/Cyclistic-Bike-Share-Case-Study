...
```{r}
#Line graph of number of bike rides per group per day (Graph #1)
line_bikerides_per_group_per_day <- grouped_data_week %>% ggplot(aes(x = day_of_week, y = number_of_rides, group = member_casual, color = member_casual)) + geom_line() + labs(title = "Number of Rides per Day for each Member Type", x = "Day of Week", y = "Number of Rides")
line_bikerides_per_group_per_day
```

```{r}
#Bar graph of number of bike rides per group per day (Graph #2)
...
```
```{r}
#Line graph of average length of bike rides per group per day
avg_length_line_bikerides_per_group_per_day <- grouped_data_week %>% ggplot(aes(x = day_of_week, y = average_duration, group = member_casual, color = member_casual)) + geom_line() + labs(title = "Average Duration of Rides per Day for each Member Type", x = "Day of Week", y = "Average Duration")
avg_length_line_bikerides_per_group_per_day
```

```{r}
#Bar graph of average length of bike rides per group per day (Graph #3)
...
```{r}
#number of rides per month for each member type
...
```{r}
#Most frequent bike ride starting hour for each member type
